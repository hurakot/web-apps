...
#-----------------------------------------------
#Все используемые в веб-приложении таблицы из БД.
#-----------------------------------------------
...
# Данная таблица хранит записи всех пользователей и используется для авторизации пользователей
...
class Course(Base):
    ...
    rating_sum: Mapped[int] = mapped_column(default=0) #Сумма значений оценок курса
    rating_num: Mapped[int] = mapped_column(default=0) #Количество оценок, проставленных курсу
...
class Review(Base):
    ...
    rating: Mapped[int] = mapped_column(default=0, nullable=False) #Оценка, которую пользователь проставил курсу (целое число от 0 до 5)
    ...
    course_id: Mapped[int] = mapped_column(ForeignKey("courses.id"), nullable=False) #Идентификатор курса
    user_id: Mapped[int] = mapped_column(ForeignKey("users.id"), nullable=False) #Идентификатор пользователя
